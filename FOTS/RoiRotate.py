...
class RoiRotate(object):
    ...
    def scanFunc(self, state, b_input):
        ...
        # resize keep ratio
        w = tf.cast(tf.ceil(tf.multiply(tf.divide(self.fix_RoiHeight, cropBox[3]), tf.cast(cropBox[2], tf.float64))),
                    tf.int32)
        ...
        # pad
        pad_or_crop_textImgFeatures = tf.image.pad_to_bounding_box(pad_or_crop_textImgFeatures, 0, 0,
                                                                   self.fix_RoiHeight, self.max_RoiWidth)
