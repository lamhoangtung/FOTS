...
def build_model(opt, reuse_variables, scope=None):
    ...
    input_score_maps = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, None, None, 1],
    ...
    input_training_masks = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, None, None, 1],
    ...
    total_loss, model_loss, detector_loss, recognizer_loss = fots.total_loss(input_score_maps, input_geo_maps,
                                                                             input_training_masks, input_btags,
                                                                             input_recg_masks)
...
def main(argv=None):
    ...
    tower_total_loss, tower_model_loss, tower_detector_loss, tower_recognizer_loss, tower_batch_norm_updates_op, tower_grads = list(
        zip(
            *models))[-6:]
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            count = 0
            for m in models:
                input_images, input_score_maps, input_geo_maps, input_training_masks, input_brboxes, input_btags, input_brecg_masks = m[
                                                                                                                                      :7]
            ...
            dl, rl, ml, tl, _ = sess.run([detector_loss, recognizer_loss, model_loss, total_loss, train_op],
                                         feed_dict=inp_dict)
            if np.isnan(tl):
            ...
            if step % 10 == 0:
                ...
                avg_examples_per_second = (10 * FLAGS.batch_size_per_gpu * (len(gpus) if gpus else 1)) / (
                        time.time() - start)
                start = time.time()
                print(
                    'Step {:06d}, total loss {:.4f}, detector loss {:.4f}, recognizer loss {:.4f}, model loss {:.4f}, {:.2f} seconds/step, {:.2f} examples/second'.format(
            ...
            if step % FLAGS.test_steps == 0 and FLAGS.test_steps > 0:
                results = fots_testModel.detectRecg(d_images, sess=sess)
